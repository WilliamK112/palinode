"""
Tests for the /wrap push contract.
...
own ``.claude/commands/wrap.md`` copy (ADR-011 update), so these tests
...
def test_wrap_body_calls_palinode_push_before_session_end():
    """WRAP_COMMAND_BODY must call palinode_push before palinode_session_end."""
    body = _wrap_body()
...
def test_wrap_body_surfaces_non_remote_push_failures():
    """WRAP_COMMAND_BODY must instruct the agent to surface push failures."""
    body = _wrap_body()
    has_error_surface = (
...
def test_wrap_session_end_pushes_its_own_note():
    ...
    The wrap push-before-archive change added push-before-archive (sync prior commits).
    The session-end hook audit originally added a SECOND trailing palinode_push because
    session_end committed the daily note WITHOUT pushing it (config.git.auto_push
    defaults to False), so a push-before-only flow left the wrap's own note unpushed —
    the final session before a gap never reached the remote.

    The trailing push was a forgettable third prose step. It is now folded into
    session_end via the `push` parameter (the session-end hook audit follow-up):
    `session_end(push=true)` commits AND pushes the note in one call, making the
    note-ship a structural property of the tool rather than something the agent must
    remember. This test guards that the wrap body actually passes push: true. """
    body = _wrap_body()
